...
export default function Navbar({ className }: { className?: string }) {
    ...
    const [image, setImage] = useState<string | undefined | null>(""); // Provide a default value of null

    const setImageUrl = async () => {
        setImage(await getUserProfileImage(Number(user?.sub)));
    }   
    ...
    return (
        <div className={cn(`px-8 inset-x-0 -mx-10 sm:mx-auto backdrop-blur-sm bg-slate-50`, className)}>
            <Menu setActive={setActive}>
                <div className="flex justify-between items-center w-full">
                    ...
                    <div className="flex items-center space-x-4">
                        {
                            pathname === "/" ? (<div className="flex max-w-xs items-center space-x-2 sm:block  hidden ">

                            <Button className=" text-neutral-900 bg-white hover:bg-gray-100" > <CommandSearch /> </Button>
                        </div>) : null
                        }
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent>
                                ...
                                {user && <Link href={`/viewprofile/${Number(user.sub)}`} ><DropdownMenuItem>Profile </DropdownMenuItem></Link>}
                                <Link href="/drafts" >
